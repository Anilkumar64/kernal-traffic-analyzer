                               ...
                               const QVector<TrafficEntry> &conns) const
{
    ...
    if (proc.exe.startsWith("/tmp") || proc.exe.startsWith("/dev/shm")
        || proc.exe.startsWith("/run/user")) {
        ts.score -= 30;
    ...
    if (totalRate > 10 * 1024 * 1024) {  // >10 MB/s
        ts.score -= 10;
    ...
    // Grade
    if      (ts.score >= 90) { ts.grade = "A"; ts.color = QColor("#20d060"); }
    else if (ts.score >= 75) { ts.grade = "B"; ts.color = QColor("#3fb950"); }
    else if (ts.score >= 60) { ts.grade = "C"; ts.color = QColor("#f0b800"); }
    else if (ts.score >= 40) { ts.grade = "D"; ts.color = QColor("#f07000"); }
    else                     { ts.grade = "F"; ts.color = QColor("#f04040"); }
                            ...
                            const QString &reason)
{
    ...
    e.manual  = true;
    switch (level) {
        case TrustLevel::Trusted:    e.score = 90; break;
        case TrustLevel::Neutral:    e.score = 50; break;
        case TrustLevel::Suspicious: e.score = 20; break;
        case TrustLevel::Blocked:    e.score = 0;  break;
        default:                     e.score = 50; break;
    }
    m_entries[process] = e;
...
QString TrustScorer::labelForLevel(TrustLevel l)
{
    switch (l) {
        ...
        case TrustLevel::Suspicious: return "Suspicious";
